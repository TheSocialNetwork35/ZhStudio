...
- Die automatische Browser-Sprachauswahl gilt nur beim ersten Einstieg über `/` pro Browser-Tab. Ein sessionStorage-Marker verhindert weitere automatische Weiterleitungen in diesem Tab; wer `/fr` entfernt, bleibt auf Deutsch. Ein neuer Tab erkennt die Browsersprache erneut. Direkt aufgerufene Unterseiten bleiben in der Sprache ihrer URL. Die Sprache direkt aufgerufener Unterseiten wird nur für den aktuellen Tab gespeichert; eine explizite Auswahl ist über `?lang=de` oder `?lang=fr` möglich. Ein sichtbarer Sprachschalter wird nicht angezeigt. Der Server liefert für jede URL unabhängig von der Browser-Sprache dieselben Inhalte.
