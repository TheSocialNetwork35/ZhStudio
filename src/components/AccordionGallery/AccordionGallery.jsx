...
export default function AccordionGallery({
  ...
  const pointerWasActiveRef = useRef(null)
  ...
  const applyLayout = useCallback((animate) => {
    ...
    panels.forEach((panel, index) => {
      ...
      timeline.to(panel, { flexGrow: isActive ? grow : 1, ...rotationProperty, duration: animationDuration, ease }, 0)
      ...
      if (showLabels && bar && text) {
        if (isActive) {
          timeline.to([bar, text], { opacity: 1, x: 0, duration: animationDuration, ease, stagger: prefersReduced ? 0 : stagger }, 0)
        } else {
          timeline.to([bar, text], { opacity: 0, x: -14, duration: animationDuration * 0.6, ease }, 0)
        }
      }
    })
  ...
  const handleClick = (index, event) => {
    const pointerWasActive = pointerWasActiveRef.current
    pointerWasActiveRef.current = null
    if (pointerWasActive === false || (pointerWasActive === null && index !== active)) {
      ...
      {items.map((item, index) => {
        ...
        return (
          <Tag
            ...
            onPointerDown={() => { pointerWasActiveRef.current = index === active }}
